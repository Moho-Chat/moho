...
/// daemon/chatd/model.c's chatd_account_json) - the frontend renders these
...
/// Matches Buffer JSON (daemon/chatd/model.c's chatd_buffer_json).
...
/// Matches Message JSON (daemon/chatd/uiops_conv.c + store.c's row shape).
#[derive(Serialize, Clone, Debug)]
pub struct Message {
    pub id: String,
...
/// A fresh, process-unique message id (daemon/chatd/model.c's
/// chatd_next_message_id: "<unix-ts>.<seq>").
...
/// "~"/"@"/"%"/"+"/"" by descending rank (daemon/chatd/model.c's
/// chatd_member_prefix).
