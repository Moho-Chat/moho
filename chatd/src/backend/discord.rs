...
async fn run_qr_login(state: &AppState, login_id: &str) -> Result<()> {
    ...
    let qr_path = std::env::temp_dir().join(format!("moho-discord-qr-{login_id}.png"));
...
/// yet. Unlike IRC (which has no server-side history at all - chatd only
...
/// potentially weeks ago just because this is the first time chatd has
/// seen the channel.
...
async fn run_gateway(state: &AppState, config: &DiscordAccountConfig) -> Result<()> {
    ...
    out_tx.send(
        json!({
            ...
            "d": {
                "token": config.token,
                "properties": { "os": "linux", "browser": "chatd", "device": "chatd" },
                "compress": false,
                "large_threshold": 50,
            }
        })
