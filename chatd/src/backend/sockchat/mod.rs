...
fn avatar_cache_dir() -> std::path::PathBuf {
    dirs::cache_dir().unwrap_or_else(|| dirs::home_dir().unwrap_or_default().join(".cache")).join("moho").join("sockchat-avatars")
}
...
/// Where cached attachment images/video live - same `~/.cache/moho`
...
fn attachment_cache_dir() -> std::path::PathBuf {
    dirs::cache_dir().unwrap_or_else(|| dirs::home_dir().unwrap_or_default().join(".cache")).join("moho").join("sockchat-attachments")
}
...
mod live_probe {
    ...
    async fn sockchat_http_probe() {
        ...
        let dir = std::env::temp_dir().join("chatd-tor-spike");
