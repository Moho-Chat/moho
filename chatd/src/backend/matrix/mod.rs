    ...
    loop {
        ...
        let detail = match result {
            ...
            Err(_) => {
                ...
                "internal error (see chatd logs)".to_string()
            }
        };
        state.runtime.set_conn_state(state, account_id, ConnState::Connecting, None);
...
fn config_dir() -> std::path::PathBuf {
    dirs::home_dir().unwrap_or_default().join(".config").join("moho")
}
...
/// so, since those are always chatd's own already-fetched output rather
...
/// config_dir()'s `~/.config/moho` (see main.rs's own
...
fn media_cache_dir() -> std::path::PathBuf {
    dirs::cache_dir().unwrap_or_else(|| dirs::home_dir().unwrap_or_default().join(".cache")).join("moho").join("matrix-media")
}
...
pub fn start_login(state: AppState, login_id: String, homeserver_url: String, username: String, password: String) {
    tokio::spawn(async move {
        ...
        let error = match result {
            Ok(Ok(())) => return,
            Ok(Err(e)) => format!("{e:#}"),
            Err(_) => "internal error (see chatd logs)".to_string(),
        };
...
mod tests {
    ...
    async fn matrix_end_to_end_probe() {
        ...
        let create_resp = http::post_json(
            ...
            serde_json::json!({ "preset": "private_chat", "name": "chatd-matrix-phase2-probe" }),
        )
        .await
        ...
        let data_dir = std::env::temp_dir().join(format!("chatd-matrix-probe-{}", std::process::id()));
        ...
        let buffer_name = "chatd-matrix-phase2-probe";
    ...
    /// chatd's own sync loop against it (as one "device" of the test
    ...
    async fn matrix_e2ee_read_probe() {
        ...
        let create_resp = http::post_json(
            ...
            serde_json::json!({
                "preset": "private_chat",
                "name": "chatd-matrix-phase3-probe",
                "initial_state": [
        ...
        let data_dir = std::env::temp_dir().join(format!("chatd-matrix-e2ee-probe-{}", std::process::id()));
        std::fs::create_dir_all(&data_dir).unwrap();
        let state = AppState {
            store: std::sync::Arc::new(crate::store::Store::open(&data_dir.join("scrollback.db")).expect("opening store")),
            accounts: std::sync::Arc::new(crate::accounts::AccountStore::open(data_dir.join("accounts.toml")).expect("opening accounts")),
        ...
        let buffer_name = "chatd-matrix-phase3-probe";
        let mut buffer_id = None;
        for _ in 0..30 {
            tokio::time::sleep(Duration::from_secs(1)).await;
            if let Some(b) = state.runtime.list_buffers().into_iter().find(|b| b.name == buffer_name) {
    ...
    /// second client needed): "device A" is chatd's own real production
    ...
    /// pass, exactly the "chatd sends, a different real client decrypts"
    ...
    async fn matrix_e2ee_two_device_roundtrip() {
        ...
        // --- Device A: chatd's own real connect loop ---
        ...
        let create_resp = http::post_json(
            ...
            serde_json::json!({
                "preset": "private_chat",
                "name": "chatd-matrix-phase34-probe",
                "initial_state": [
                    { "type": "m.room.encryption", "state_key": "", "content": { "algorithm": "m.megolm.v1.aes-sha2" } }
                ],
            }),
        ...
        let data_dir = std::env::temp_dir().join(format!("chatd-matrix-e2ee-roundtrip-{}", std::process::id()));
        std::fs::create_dir_all(&data_dir).unwrap();
        let state = AppState {
            store: std::sync::Arc::new(crate::store::Store::open(&data_dir.join("scrollback.db")).expect("opening store")),
            accounts: std::sync::Arc::new(crate::accounts::AccountStore::open(data_dir.join("accounts.toml")).expect("opening accounts")),
        ...
        // from the real ~/.config/moho (not this test's isolated
        ...
        let crypto_dir = dirs::home_dir().unwrap_or_default().join(".config").join("moho").join("matrix-crypto");
        ...
        let buffer_name = "chatd-matrix-phase34-probe";
        let mut buffer_id = None;
        for _ in 0..30 {
            tokio::time::sleep(Duration::from_secs(1)).await;
            if let Some(b) = state.runtime.list_buffers().into_iter().find(|b| b.name == buffer_name) {
    ...
    async fn matrix_phase5_polish_probe() {
        ...
        let create_resp = http::post_json(
            &format!("{}/_matrix/client/v3/createRoom", homeserver.trim_end_matches('/')),
            Some(&login.access_token),
            serde_json::json!({ "preset": "private_chat", "name": "chatd-matrix-phase5-probe" }),
        )
        .await
        .expect("createRoom failed");
        ...
        let data_dir = std::env::temp_dir().join(format!("chatd-matrix-phase5-probe-{}", std::process::id()));
        std::fs::create_dir_all(&data_dir).unwrap();
        let state = AppState {
            store: std::sync::Arc::new(crate::store::Store::open(&data_dir.join("scrollback.db")).expect("opening store")),
            accounts: std::sync::Arc::new(crate::accounts::AccountStore::open(data_dir.join("accounts.toml")).expect("opening accounts")),
        ...
        let buffer_name = "chatd-matrix-phase5-probe";
        let mut buffer_id = None;
        for _ in 0..30 {
            tokio::time::sleep(Duration::from_secs(1)).await;
            if let Some(b) = state.runtime.list_buffers().into_iter().find(|b| b.name == buffer_name) {
    ...
    async fn matrix_media_probe() {
        ...
        let create_resp = http::post_json(
            &format!("{}/_matrix/client/v3/createRoom", homeserver.trim_end_matches('/')),
            Some(&login.access_token),
            serde_json::json!({ "preset": "private_chat", "name": "chatd-matrix-media-probe" }),
        )
        .await
        .expect("createRoom failed");
        let room_id = create_resp["room_id"].as_str().expect("no room_id").to_string();
        ...
        let data_dir = std::env::temp_dir().join(format!("chatd-matrix-media-probe-{}", std::process::id()));
        std::fs::create_dir_all(&data_dir).unwrap();
        let state = AppState {
            store: std::sync::Arc::new(crate::store::Store::open(&data_dir.join("scrollback.db")).expect("opening store")),
            accounts: std::sync::Arc::new(crate::accounts::AccountStore::open(data_dir.join("accounts.toml")).expect("opening accounts")),
        ...
        let buffer_name = "chatd-matrix-media-probe";
        let mut buffer_id = None;
        for _ in 0..30 {
            tokio::time::sleep(Duration::from_secs(1)).await;
            if let Some(b) = state.runtime.list_buffers().into_iter().find(|b| b.name == buffer_name) {
    ...
    fn matrix_encrypted_media_json_round_trip() {
        ...
        let plaintext = b"chatd encrypted attachment round-trip probe".to_vec();
