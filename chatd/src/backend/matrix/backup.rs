...
mod tests {
    ...
    /// under the real `~/.config/moho`, keyed only by account_id (no
    ...
    async fn login_device(homeserver: &str, username: &str, password: &str, tag: &str) -> TestDevice {
        ...
        let data_dir = std::env::temp_dir().join(format!("chatd-matrix-backup-probe-{tag}-{}", std::process::id()));
    ...
    async fn matrix_backup_restore_probe() {
        ...
        let create_resp = http::post_json(
            ...
            serde_json::json!({
                "preset": "private_chat",
                "name": "chatd-matrix-backup-probe",
